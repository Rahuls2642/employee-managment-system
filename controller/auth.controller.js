...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    const existing = await db.query(` SELECT * FROM users WHERE mobile_no=$1`, [
      mobile_no,
    ]);
    if (existing.rows.length > 0) {
      return res
        .status(400)
        .json({
          error: "Mobile no already registeres use diffrent mobile number",
        });
    }
...
export const loginUser = async (req, res) => {
  try {
    ...
    const result = await db.query(` SELECT * FROM users WHERE mobile_no=$1`, [
      mobile_no,
    ]);
